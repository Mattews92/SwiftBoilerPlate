...
@objc protocol ViewControllerDelegate {
    @objc optional func controllerPopped()
    @objc optional func messageReceived(message: String)
    @objc optional func dataReceivedOnSocket(data: [Any]?)
}
...
extension UIViewController {
    ...
    func showAlertWithTitle(_ title: String?,
        ...
        DispatchQueue.main.async {
            ...
            /*appViewController?*/self.present(alert,
                                           animated: true,
                                           completion: nil)
        }
    }
    ...
    func showActivityIndicator(drawOnSelf onSelf: Bool = false){
        ...
        if (self.indicatorRetainCount ?? 0) <= 1 && self.activityOverlayView == nil {
            ...
            DispatchQueue.main.async {
                self.addActivityIndicator(drawOnSelf: onSelf)
            }
        }
    }
    ...
    private func addActivityIndicator(drawOnSelf onSelf: Bool) {
        ...
        if onSelf {
            baseLineView = self.view
        }
        else {
            let window = UIApplication.shared.delegate?.window!!
            baseLineView = window!.forFirstBaselineLayout
        }
        ...
        if !(self.activityOverlayView?.isDescendant(of: baseLineView) ?? false) {
